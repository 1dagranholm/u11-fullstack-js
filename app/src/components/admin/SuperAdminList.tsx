...
class SuperAdminList extends React.Component<RouteComponentProps, IState> {
    ...
    public deleteUser(id: number) {
        axios.delete(`http://localhost:8080/api/users/${id}`).then((response) => {
            const userIndex = this.state.superadmin.findIndex((user) => user._id === id);
            let users = [...this.state.superadmin];
            let superadmin = { ...users[userIndex] };
            superadmin.deletedAt = response.data.data.deletedAt;
            users[userIndex] = superadmin;
            superadmin = users;
    ...
    public render() {
        const users = this.state.superadmin;
